 ...
 * @since      1.0.0
 */
class Instagram_Channel extends Abstract_Messaging_Channel {
    ...
    public function initialize($settings) {
        ...
        if (empty($settings['account_id']) || empty($settings['access_token'])) {
            $this->logger->warning('Instagram channel initialized with missing required settings', array(
    ...
    public function process_webhook($data) {
        ...
        try {
            ...
            $this->logger->error('Error processing Instagram webhook', array(
                'error' => $e->getMessage()
            ));
    ...
    /**
     * Process an individual messaging event from Instagram.
     *
     * @since    1.0.0
    ...
    /**
     * Process an incoming message from Instagram.
     *
     * @since    1.0.0
    ...
    private function process_incoming_message($event) {
        ...
        if (isset($event['message']['text'])) {
            ...
            $message_type = 'text';
        } elseif (isset($event['message']['attachments']) && is_array($event['message']['attachments'])) {
            // Media message - we'll just extract the attachment type
            $attachment = $event['message']['attachments'][0] ?? null;
            $message_type = $attachment['type'] ?? 'unknown';
            
            // For now, we'll just acknowledge non-text messages
        ...
        if ($message_type === 'text') {
            ...
            // For non-text messages, send a canned response
            $response = $this->send_message($sender_id, sprintf(
                __('I received your %s, but I can only respond to text messages at this time.', 'conversaai-pro-wp'),
                $message_type
            ));
    ...
    private function handle_verification_request($data) {
        ...
        // Get app secret from saved settings
        $app_secret = '';
    ...
    public function test_connection() {
        if (!$this->is_available()) {
            return array(
                'success' => false,
                'error' => 'Instagram channel is not properly configured'
            );
        }
        ...
        return array(
            ...
            'details' => array(
                ...
                'profile_picture_url' => $response_body['profile_picture_url'] ?? ''
            )
        );
    }
